...
exports.PersonalityComponent = exports.Personality = exports.Kryten = void 0;
...
class Kryten {
  ...
  async say(speakable) {
    ...
    await _speech.Speech.speak(speakable, {
      voice
    });
  }
  async hesl(greeting) {
    await this.ready;
    const voice = this.currentVoice();
    await _speech.Speech.speak(greeting, {
      voice
    });
    let command = null;
    let running = true;
    ...
    const listenAndRespond = async () => {
      if (!running) return;
      await _speech.Speech.listen(async spoken => {
        this.chatlog.push({
          user: 'user',
          message: spoken
        });
        ...
        if (command = this.parseCommand(spoken)) {
          ...
          await _speech.Speech.speak(res, {
            voice
          });
        }
        if (!running) return;
